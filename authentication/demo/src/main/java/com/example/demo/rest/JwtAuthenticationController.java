...
public class JwtAuthenticationController {
    ...
    public ResponseEntity<?> saveUser(@Valid @RequestBody UserDTO user) throws Exception {
        ...
        BufferedImage bi = ImageIO.read(new File("C:\\Users\\Mohamed Marzouk\\Desktop\\V5MadaLore\\pictures\\user.jpg"));
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        ImageIO.write(bi, "jpg", baos);
        byte[] bytes = baos.toByteArray();

        ImageDTO imageDTO = new ImageDTO("user.jpg", ImageUtility.compressImage(bytes),user,"image/jpeg");
    ...
    //add mapping for PUT /users - update existing user's email
    ...
    //add mapping for PUT /users - update existing user's password
